...
class Model(sampling.TransitionBatchUpdate, BaseDeepAgent):
    ...
    def reinforce_(self, state, action, reward, next_state,
        ...
        super(Model, self).reinforce_(state, action, reward, next_state, episode_done, **kwargs)
    ...
    def set_session(self, sess):
        super(Model, self).set_session(sess)
        self.network.set_session(sess)

    def init_network(self, f_se, f_transition, f_decoder, state_shape, num_actions, *args, **kwargs):
        def f(inputs):
        ...
        return network.Network([input_state, input_action, input_frame], f, var_scope="learn")
