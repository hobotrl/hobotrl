...
def func_compile_exp_agent(state, action, rewards, next_state, done):
    ...
    road_invalid = rewards[0] > 1.99  # mostly red
...
def gen_backend_cmds():
    ...
    backend_cmds = [
        ...
        ['python', utils_path+'gen_launch_dynamic_v1.py',
         ...
         utils_path+'honda_dynamic_obs_template_tilt.launch', 8],
        # start roscore
        ['roscore'],
...
env = KubernetesEnv(
    remote_client_env_class=DrivingSimulatorEnv,
    image_uri="docker.hobot.cc/carsim/simulator_gpu_kub:0.0.6_384.90",
    # address="vmgpu016.hogpu.cc", port='6002',
...
op_global_step_inc = tf.assign_add(global_step, 1)
op_global_step_reset = tf.assign(global_step, 0)
...
memory = Playback(capacity=traj_count)
# memory = BigPlayback(
#     bucket_cls=Playback,
...
#     capacity=traj_count,
#     active_ratio=ratio,
#     cache_path="./ReplayBufferCache/inference",
# )
...
def log_info(update_info):
    ...
    if cnt_skip == 0 or n_steps == 0:
        ...
        print p_str
        if phase_infer is not None:
            summary_proto.value.add(tag='phase_infer', simple_value=phase_infer)
    ...
    if done:
        ...
        if not exploration_off:
            ...
            summary_proto.value.add(
                tag='flag_success_noexplore', simple_value=flag_success)
...
try:
    ...
    with sv.managed_session(config=config) as sess:
        ...
        sess.run(op_global_step_reset)
        while True:
            n_ep += 1
            sess.run(op_global_step_inc)
            ...
            state  = env.reset()
            phase_infer = time.time() % (n_skip * 0.5)
            ...
            while True:
                ...
                flag_tail = done
                flag_success = True if flag_success and reward > 0.1 else False
                skip_reward += reward
                ...
                if cnt_skip==0 or done:
                    ...
                    if ema_speed < 2.0 and ema_dist < 1.0 and next_action != 0:
                        print "Stop rule correction to lane keep"
                        next_action = 0
                    t_infer += time.time() - t
                    skip_reward = 0
