...
tf.app.flags.DEFINE_string(
    "dir_prefix", "../../../../../../work/agents/Compare/AgentStepAsCkpt/exp20/2/",
...
class FuncReward(object):
    ...
    def _func_scalar_reward(self, rewards, action):
        ...
        reward = [
            ...
            -100.0 * obs_risk,
            # opposite
            -400 * (0.9 + 0.1 * mom_opp) * (mom_opp > 0.99),
            # ped
            -400 * (0.9 + 0.1 * mom_biking) * (mom_biking > 0.99),
            # steer
            steering * -40.0,
        ]
...
try:
    ...
    with __agent.create_session(
        ...
        while n_total_steps <= 2.5e5:
            ...
            state = env.reset()
            while True:
                action = agent.act(state, exploration=not FLAGS.test)
                if action != 3:
                    print_qvals(
