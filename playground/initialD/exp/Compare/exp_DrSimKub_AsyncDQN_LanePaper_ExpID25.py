...
def exp(dir_prefix, tf_log_dir="ckpt", our_log_dir="logging", replay_cache_dir="ReplayBufferCache",
    ...
    class FuncReward(object):
        ...
        def _func_scalar_reward(self, rewards, action):
            ...
            road_change = rewards[4] > 0.01  # entering intersect
            # ion
            opp = rewards[5]
            biking = rewards[6]
            # inner = rewards[7]
        ...
        with __agent.create_session(
            ...
            while n_total_steps <= 2.5e5:
                ...
                while True:
                    ...
                    if vec_reward is not None and action != 3:
                        if reward_vector2scalar._ema_speed < 2.0 and reward_vector2scalar._ema_dist < 1.0:
                            action = 0
