...
class DiscreteActorCriticUpdater(network.NetworkUpdater):
    def __init__(self, policy_dist, q_function, target_estimator, entropy=1e-3, max_advantage=10.0):
        ...
        print "self._input_target_q, self._input_action, self._input_entropy, td, advantage, pi_loss:", self._input_target_q, self._input_action, self._input_entropy, td, advantage, pi_loss
...
class DiscretePGUpdater(network.NetworkUpdater):
    def __init__(self, policy_dist, v_function, target_estimator, entropy=1e-3, max_advantage=10.0):
        ...
        print "self._input_target_q, self._input_action, self._input_entropy, advantage, pi_loss:", self._input_target_v, self._input_action, self._input_entropy, advantage, self._pi_loss
        self._update_operation = network.MinimizeLoss(self._op_loss,
...
class ActorCritic(sampling.TrajectoryBatchUpdate,
    ...
    def __init__(self,
                 ...
                 # ACUpdate arguments
                 num_actions, discount_factor, entropy=1e-3, target_estimator=None, max_advantage=10.0,
                 # optimizer arguments
        ...
        if target_estimator is None:
            target_estimator = target_estimate.NStepTD(self._q_function, discount_factor)
        self.network_optimizer = network_optimizer
        network_optimizer.add_updater(
            DiscreteActorCriticUpdater(self._pi_distribution, self._q_function, target_estimator, entropy), name="ac")
    ...
    def set_session(self, sess):
        ...
        self._q_function.set_session(sess)
        self._pi_function.set_session(sess)
...
if __name__ == '__main__':
    ...
    def f_net(inputs):
        ...
        pi = network.Utils.layer_fcs(state, [200, 100], env.action_space.n, activation_out=tf.nn.softmax, l2=l2, var_scope="pi")
        return {"q": q, "pi": pi}
    ...
    sv = agent.init_supervisor(
        ...
        init_op=tf.global_variables_initializer(), save_dir="ac_new"
    )
    with sv.managed_session(config=config) as sess:
        agent.set_session(sess)
        runner = hrl.envs.EnvRunner(
            env, agent, evaluate_interval=sys.maxint,
            render_interval=sys.maxint, logdir="ac_new"
        )
        runner.episode(5000)