...
class Utils(object):
    ...
    def abs_var_scope(var_scope):
        return tf.get_variable_scope().name + "/" + var_scope
...
class Network(object):
    ...
    def __init__(self, inputs, network_creator, var_scope, name_scope=None, reuse=False):
        ...
        :param reuse:
        """
        print "Network var_scope:", var_scope
        ...
        self._variables = Utils.scope_vars(self._var_scope)
...
class LocalOptimizer(NetworkOptimizer):
    """
...
class DistributedOptimizer(NetworkOptimizer):
    """
    ...
    def apply_gradients_op_(self, grads_and_vars):
        for i, (grad, var) in enumerate(grads_and_vars):
            grads_and_vars[i] = (grad, self._var_map[var])
        apply_op = self._global_optimizer.apply_gradients(grads_and_vars)
