    ...
    def __init__(self, env=None, f_se=None, f_ac=None, f_forward=None, f_inverse=None, episode_n=10000,
        ...
        if (f_forward and f_se and f_inverse and f_ac) is None:
            ...
            def create_ac(inputs):
                l2 = 1e-7
                se = inputs[0]
                v = hrl.utils.Network.layer_fcs(se, [9216], 1,
                                                activation_hidden=tf.nn.relu,
                                                l2=l2,
                                                var_scope="v")
                v = tf.squeeze(v, axis=1)
                pi = hrl.utils.Network.layer_fcs(se, [9216], dim_action,
                                                 activation_hidden=tf.nn.relu,
                                                 activation_out=tf.nn.softmax,
                                                 l2=l2,
                                                 var_scope="pi")
            ...
            def create_forward(inputs):
                ...
                phi2_hat = hrl.utils.Network.layer_fcs(f, [9216], 9216,
                                                       activation_hidden=tf.nn.relu,
            ...
            def create_inverse(inputs):
                ...
                logits = hrl.utils.Network.layer_fcs(g, [9216], dim_action,
                                                     activation_hidden=tf.nn.relu,
                                                     activation_out=tf.nn.relu,
                                                     l2=l2,
        ...
        super(A3CCarDiscreteICM, self).__init__(env, f_se, f_ac, f_forward, f_inverse, episode_n, learning_rate,
                                                discount_factor, entropy, batch_size)


Experiment.register(A3CCarDiscreteICM, "A3C with ICM for CarRacing")
