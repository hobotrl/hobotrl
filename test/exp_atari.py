    ...
    def run(self, args):
        ...
        if args.job == "ps":
            ...
            def create_ac_atari(input_state, num_action, **kwargs):
                se = hrl.utils.Network.conv2ds(input_state, shape=[(32, 8, 4), (64, 4, 2), (64, 3, 1)],
                                               out_flatten=True,
                                               activation_hidden=tf.nn.relu, var_scope="se")

                q = hrl.utils.Network.layer_fcs(se, [256], num_action, activation_hidden=tf.nn.relu, var_scope="q")
                pi = hrl.utils.Network.layer_fcs(se, [256], num_action, activation_hidden=tf.nn.relu,
                                                 activation_out=tf.nn.softplus, var_scope="pi")

                pi = tf.nn.softmax(pi)
                pi = pi + prob_min
                pi = pi / tf.reduce_sum(pi, axis=-1, keep_dims=True)
                r = hrl.utils.Network.layer_fcs(se, [256], 1, activation_hidden=tf.nn.relu, var_scope="r")
            ...
            with tf.device("/job:worker/task:0"):
                ...
                global_net = a3coo.ActorCritic(0, "global_net", state_shape, env.action_space.n,
                                               create_ac_atari, optimizer=optimizer(), global_step=global_step, **kwargs)
